...
///
/// 1. **Test phase**: `test_set` is called for ALL varbinds before any commits.
///    If any test fails, no changes are made.
///
/// 2. **Commit phase**: `commit_set` is called for each varbind in order.
///    If a commit fails, `undo_set` is called for all previously committed
