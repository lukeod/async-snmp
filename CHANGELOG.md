...
- The production GETBULK decode path (community and v3) did not range-check `non-repeaters`/`max-repetitions`; negatives are now clamped to 0 in `Pdu::decode` (net-snmp-compatible, matching the agent's own normalization, RFC 3416 Section 4.2.3), and the previously test-only strict decoder was removed so there is a single code path. `Pdu::encode` also clamps these fields for directly-constructed PDUs, making encode a genuine choke point.
...
- GETBULK's client-side encode path could put a negative `non-repeaters` or `max-repetitions` on the wire even though RFC 3416 Section 4.2.3 specifies both as `INTEGER (0..2147483647)`. Standard and GETBULK PDUs now share one typed `Pdu` representation and policy-aware codec, so community and v3 wrappers apply identical normalization or strict rejection. The walk paths also use a saturating conversion (to `i32::MAX`) instead of wrapping configured `u32` values into negative `i32` values.
